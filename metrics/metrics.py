...
class Metrics:
  ...
  def __init__(self,path_fake,path_real=None,model="inception",path_model=None,splits=10,object_names=None):
...
if __name__ == '__main__': 
  ...
  scores [string] (optional)=comma separated string of score abbreviations without any space like "fid,mis,is,ds,cas,ams" (defaults to all scores)
  path_result [string] (optional)="path/to/result.csv" [if not provided then a suitable path is inferred]

  sample-use:
  python 'path/to/metrics.py' "$path_fake" --path_real "$path_real" --splits="$splits" --model "$model" --object_names "$object_names" --path_model "$path_model" --path_result "$path_result" --precision="$precision"
  ...
  metric=Metrics(path_fake,path_real,model,path_model,splits,object_names) 

  #Calculate the required scores
  scores={}
  for score_name in score_names:
    scores[score_name.upper()]=metric.score_functions[score_name]()
