...
with tf.Session() as sess:
    ...
    for object_name in object_names:
        ii = 0
        for ind in range( int(len(adj_train[object_name])/batch_size) ):
            ...
            for randome in range(50):
                np.random.seed( randome )
                z =  np.random.normal(0,1,[batch_size, latent_dim])
                lpv = sess.run(node_box_r,feed_dict= {z_latent: z,
                                                        true_class: X_in[:, :, :label_size],
                                                        class_vec:clvec})

                for bbx,pos,classix in zip(lpv, X_in[:, :, :label_size],clvec):
                    class_vec_sketch.append(classix)
                    label_vec_sketch.append(pos)
                    bbx_gen_sketch.append(((bbx)*pos)*canvas_size)

                    generated_image = plot_bbx(arrangement(((bbx)*pos)*canvas_size, object_name))
                    sza = 30
                    plt.figure(num=None, figsize=(sza, sza))
                    plt.axis('off')
                    plt.imshow(generated_image)
                    plt.savefig('../boxResults/'+object_name+'/'+str(ii)+'_'+str(randome)+'.png')
                    plt.show()
                    ii=ii+1
