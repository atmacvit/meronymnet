...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, step, path):

        ## convert tensors to numpy arrays
        visuals = self.convert_visuals_to_numpy(visuals)
                
        if False: # show images in tensorboard output
            img_summaries = []
        ...
        if self.use_html: # save images to a html file
            # print("caiscancal lkdsakcsackcnscasc",path)
            path = (path[0].strip().split('/')[-1])
            path7 = path.split('_')[0]+'/'
            # print(path)
            for label, image_numpy in visuals.items():
                if 'synthesized' in label:
                    if isinstance(image_numpy, list):
                        for i in range(len(image_numpy)):
                            ...
                            # img_path = os.path.join(self.img_dir, path7+path)
                            img_path = os.path.join('./datasets/RGB/', path)
                            util.save_image(image_numpy[i], img_path)
                            img_path1 = os.path.join('./datasets/RGB_met/', path7+path)
                            util.save_image(image_numpy[i], img_path1)
                    else:
                        ...
                        util.save_image(image_numpy, img_path1)
