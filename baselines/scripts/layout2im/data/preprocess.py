...
def preprocess_make_data(object_class_names, train_mode, mode='boxVAE'):
    if train_mode=='train':
     return preprocess_make_data_augment(object_class_names, train_mode)
...
def preprocess_make_data_augment(object_class_names, train_mode):
    X_train, masks, rgb_masks, rgb_imgs, class_v = make_data(object_class_names, train_mode)
    ...
    def get_key(val):
        for key, value in dog_labels.items():
            if val == value:
                return key
    for i in range(num_samples):
        data_dict[str(i)]['image'] = rgb_imgs[i]
        indices = np.where(X_train[i, :, 0] == 1)
        label_names = [get_key(val + 1) for val in indices[0]]
        labels = [(val + 1) for val in indices[0]]
        data_dict[str(i)]['boxes'] =  X_train[i][indices][:, 1:]
        ...
        data_dict[str(i)]['labels'] = labels
        data_dict[str(i)]['class'] = class_v[i]
        data_dict[str(i)]['masks'] = masks[i][indices]
        data_dict[str(i + num_samples)]['image'] = iaa.Add((-20, 20), per_channel=0.5)(image=rgb_imgs[i])
        data_dict[str(i + 2 * num_samples)]['image'] = iaa.AddToHueAndSaturation((-30, 30))(image=rgb_imgs[i])
        data_dict[str(i + 3 * num_samples)]['image'] = iaa.LinearContrast((0.5, 2.0), per_channel=0.5)(image=rgb_imgs[i])
        data_dict[str(i + num_samples)]['label_names'] = data_dict[str(i)]['label_names']
        data_dict[str(i + num_samples)]['boxes'] = data_dict[str(i)]['boxes']
        data_dict[str(i + num_samples)]['labels'] = data_dict[str(i)]['labels']
        data_dict[str(i + num_samples)]['class'] = data_dict[str(i)]['class']
        data_dict[str(i + num_samples)]['masks'] = data_dict[str(i)]['masks']
        data_dict[str(i + 2 * num_samples)]['label_names'] = data_dict[str(i)]['label_names']
        data_dict[str(i + 2 * num_samples)]['boxes'] = data_dict[str(i)]['boxes']
        data_dict[str(i + 2 * num_samples)]['labels'] = data_dict[str(i)]['labels']
        data_dict[str(i + 2 * num_samples)]['class'] = data_dict[str(i)]['class']
        data_dict[str(i + 2 * num_samples)]['masks'] = data_dict[str(i)]['masks']
        data_dict[str(i + 3 * num_samples)]['label_names'] = data_dict[str(i)]['label_names']
        data_dict[str(i + 3 * num_samples)]['boxes'] = data_dict[str(i)]['boxes']
        data_dict[str(i + 3 * num_samples)]['labels'] = data_dict[str(i)]['labels']
        data_dict[str(i + 3 * num_samples)]['class'] = data_dict[str(i)]['class']
        data_dict[str(i + 3 * num_samples)]['masks'] = data_dict[str(i)]['masks']
    ...
    print(data_dict['10']['image'].shape)
    print(data_dict['50']['image'].shape)
    print(type(data_dict['70']['image']))
    img = Image.fromarray(data_dict[str(70)]['image'])
    img.save('test_input.png')
    img = Image.fromarray(data_dict[str(70 + num_samples)]['image'])
    img.save('test_brightness.png')
    img = Image.fromarray(data_dict[str(70 + 2 * num_samples)]['image'])
    img.save('test_saturationhue.png')
    img = Image.fromarray(data_dict[str(70 + 3 * num_samples)]['image'])
    img.save('test_contrast.png')
