...
class CustomDataset(BaseDataset):
    ...
    def modify_commandline_options(parser):
        ...
        parser.set_defaults(label_nc=25) # 18 classs + null
    ...
    def __getitem__(self, index):
        ...
        object_class = torch.zeros(10)
        class_id = int(os.path.basename(label_path).split('_')[1])
        object_class[class_id] = 1.0
        flip_p = 0.0
    ...
    def get_paths(self, opt, phase):
        ...
        if phase == 'train':
            ...
            paths_by_class_labels = {}
            paths_by_class_images = {}
            for i in range(10):
              paths_by_class_labels[i] = []
              paths_by_class_images[i] = []
            ...
            #print("Total Test Samples = " + str(num_samples))
            for i in range(num_samples):
               #print(label_paths[i])
               class_idx = int(label_paths[i].split('/')[-1].split('_')[1])
               #print("Class " + str(class_idx))
               paths_by_class_labels[class_idx].append(label_paths[i])
               paths_by_class_images[class_idx].append(image_paths[i])
            for i in range(10):
               num_samples_in_class = len(paths_by_class_labels[i])
               for j in range(100):
                  if j>=num_samples_in_class:
                    idx = j%num_samples_in_class
                  else:
                    idx = j
                  final_label_paths.append(paths_by_class_labels[i][idx])
                  final_image_paths.append(paths_by_class_images[i][idx])
    ...
    def get_paths_by_label_length(self, opt, phase, label_len):
        ...
        # find all images
        image_dir = os.path.join(root, 'img2')
        label_dir = os.path.join(root, 'segmentation_map2')
    ...
    def paths_match(self, label_path, img_path):
        ...
        name1 = os.path.basename(label_path).split('_')[0]
        name2 = '%05d' % int(os.path.basename(img_path).replace('.jpg', '').split('_')[0])
        return name1 == name2
