...
order_dict = {'celebamaskhq': labels.celeba_order, 'custom':labels.custom_order, 'humanparsing': labels.humanparsing_order}
...
class SegVAEModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        elif self.opt.dataset == 'custom':
            order_str = 'custom_order'
        else:
    ...
    def forward(self, data, mode):
        ...
        if mode == 'train':
            losses, recon, gen_prior, input_semantics = self.compute_loss(real_image, input_semantics, label_set, object_class)
            return losses, recon, gen_prior, input_semantics
        elif mode == 'inference':
            with torch.no_grad():
                recon, gen_prior, input_semantics = self.inference(real_image, input_semantics, label_set, object_class)
    ...
    def compute_loss(self, real_image, input_semantics, label_set, object_name):
        losses = {}
        losses['KLD'] = 0
        ...
        for i in range(nc-1): # don't train background
            ...
            feat_prior = torch.cat([feat_prior, object_name], dim=1)
    ...
    def inference(self, real_image, input_semantics, label_set, object_class):
        recon, gen_prior = self.generate_fake(real_image, input_semantics, label_set, object_class)
        return recon, gen_prior, input_semantics

    def generate_fake(self, real_image, input_semantics, label_set, object_class):
        ...
        for i in range(nc-1):
            ...
            feat_prior = self.shapeE(x_prior, label_set, y_c)
            feat_prior = torch.cat([feat_prior, object_class], dim=1)
