...
def get_rgbmask(box, mask, images):
  ...
  for idx,img in enumerate(images):
    ...
    for j in range(24):
      ...
      try:
        ...
        part=np.zeros((64,64,3))                   #Handle exceptional cases of negaitive values in bb: make a black part rgb
        mask[idx,j,:,:,0] = np.zeros((64,64))      #Black part mask
        box[idx,j]=np.zeros(5)                     #delete gibberish bounding box-- zero out

        print('object {} at index {} skipped due to error in bounding box dimensions :({},{}),({},{})....'.format(object_name,idx,x1,x2,y1,y2))
...
def make_data():
    ...
    rgb_masks = {}
    rgbs = {}
    for object_name in ['dog']:

        with open('data/' + object_name+'_images', 'rb') as f:
          o_images = pickle.load(f)
        with open('data/' + object_name+'_part_separated_labels', 'rb') as f:
          o_labels = pickle.load(f)
        with open('data/' + object_name+'_part_separated_masks', 'rb') as f:
          o_masks = pickle.load(f)
        with open('data/' + object_name+'_part_separated_bbx', 'rb') as f:
          o_bbx = pickle.load(f)
        ...
        box, mask, rgb_mask, image = get_rgbmask(box, mask, image)
    ...
    rgbs=np.concatenate(list(rgbs.values()),axis=0)
    ...
    return X_train, masks, rgb_mask, rgbs



    #visualise(5)
def data_prep(X_train, masks, rgb_masks, point_input_size):
    ...
    points = (np.random.randint(low=0,high=63,size=(part_ae_masks.shape[0], point_input_size, 2)))
    part_ae_gt = np.zeros(shape=(part_ae_masks.shape[0], point_input_size, 1))
    
    #print(part_ae_masks.shape)
    #print(coords.shape)
    for i in range(points.shape[0]):
            part_ae_gt[i] = part_ae_masks[i, points[i,:,0], points[i,:,1], :]
    ...
    return part_ae_masks, points, part_ae_gt, part_ae_rgb_masks, masks, part_number, bce_mask, affine_input, affine_target, sign
