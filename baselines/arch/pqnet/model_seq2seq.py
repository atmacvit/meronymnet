	...
	def __init__(self, hidden_size, n_layer=1, bidirectional=False):
		...
		for i in range(n_layer):
			if bidirectional==True:
				gru_layers.append(Sequential(Bidirectional(GRU(hidden_size , return_sequences=True))))
			else:
				gru_layers.append(Sequential(GRU(hidden_size, return_sequences=True)))
...
class DecoderGRU(tf.keras.Model):
  def __init__(self, input_size, hidden_size, n_layer=1, bidirectional=False):
    ...
    for i in range(n_layer):
      if bidirectional==True:
        gru_layers.append(Bidirectional(GRU(hidden_size, return_sequences=True)))
      else:
        gru_layers.append(GRU(hidden_size, return_sequences=True))
    self.gru_layers = gru_layers
    self.linear1 = Sequential([Dense(self.n_units_hidden1), LeakyReLU(), Dense(input_size - 4)])
    self.linear2 = Sequential([Dense(self.n_units_hidden2), ReLU(), Dense(4)])
    self.linear3 = Sequential([Dense(self.n_units_hidden2)	, ReLU(), Dense(1,activation='sigmoid')])
    self.dropout_i = 0.2
    self.dropout_o = 0.2
  ...
  def call(self, inputs, hidden):
    ...
    # print("I" + str(inputs.shape))
    # print("H1" + str(hidden1.shape))
    # print("H2" + str(hidden2.shape))	
    output1 = self.gru_layers[0](inputs, initial_state=hidden1)
...
class PartSeq2Seq(tf.keras.Model):
  def __init__(self, config):
    ...
    self.part_autoencoder = PartAE(input_shape, part_en_n_layers, ef_dim, part_de_n_layers, df_dim, z_dim, is_class_conditioning=False)
    ...
    self.part_autoencoder(sample_masks)
    ...
    self.encoder =  EncoderGRU(512, n_layer=en_n_layers, bidirectional=True)
    self.decoder = 	DecoderGRU(z_dim + 4, 1024, n_layer=de_n_layers, bidirectional=False)
  ...
  def call(self, input,  training=True):
    ...
    cond = input[2]
    affine_target = tf.stop_gradient(input[3])
    target_stop = tf.stop_gradient(input[4])
    bce_mask = tf.stop_gradient(input[5])
    ...
    target_part_geo = tf.stop_gradient(part_geo_features)
    ...
    decoder_input = tf.identity(tf.tile(tf.stop_gradient(self.decoder.init_input),((batch_size, 1, 1))))
    decoder_outputs = []
    stop_signs = []
...
#print(target_stop.shape)
    bce_mask = np.ones_like(bce_mask)
    bce_loss = bce(stop_signs, target_stop) * bce_mask
    code_rec_loss = mse(decoder_outputs[:, :, :-4], target_part_geo) * bce_mask
    param_rec_loss = mse(decoder_outputs[:, :, -4:], affine_target) * bce_mask
...
# 
    # bce_loss = tf.math.maximum((tf.reduce_sum(bce_loss))/(tf.reduce_sum(bce_mask)), 0.001)
    # code_rec_loss = tf.reduce_sum(code_rec_loss)/ code_rec_loss.shape[0]
    # param_rec_loss = tf.reduce_sum(param_rec_loss)/param_rec_loss.shape[0]

    loss =  bce_loss * 0.01 + code_rec_loss + param_rec_loss
