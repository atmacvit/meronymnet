...
from arch.layout2im.bilinear import crop_bbox_batch
...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        #print("Curr state tensor shape = ", str(h_cur.shape))
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        return layer_output_list, last_stateembeddings_z_list
...
class LayoutEncoder(nn.Module):
    def __init__(self, conv_dim=64, z_dim=8, embedding_dim=64, class_num=24, resi_num=6, clstm_layers=3):
        super(LayoutEncoder, self).__init__()
        self.activation = nn.ReLU(inplace=True)
        self.embedding = nn.Embedding(class_num, embedding_dim)
        if clstm_layers == 1:
            self.clstm = LayoutConvLSTM(8, 1024, [64], (5, 5))
        elif clstm_layers == 2:
            self.clstm = LayoutConvLSTM(8, 1024, [128, 64], (5, 5))
        elif clstm_layers == 3:
            self.clstm = LayoutConvLSTM(8, 1024, [128, 64, 64], (5, 5))
        elif clstm_layers == 4:
            self.clstm = LayoutConvLSTM(8, 1024, [128, 64, 64, 64], (5, 5))
    ...
    def forward(self, objs, masks, obj_to_img, z):
        ...
        #print("Embeddings z " + str(masks.shape))
        #print("masks " + str(embeddings_z.shape))
        h = embeddings_z.view(embeddings_z.size(0), embeddings_z.size(1), 1, 1) * masks
        #print("Embeddings h " + str(h.shape))
...
class Decoder(nn.Module):
    def __init__(self, conv_dim=64):
        ...
        self.activation = nn.ReLU(inplace=True)
        # (64, 8, 8) -> (512, 8, 8)
        self.c0 = nn.Conv2d(conv_dim + 1, conv_dim * 8, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn0 = nn.BatchNorm2d(conv_dim * 8)
        # (512, 8, 8) -> (512, 16, 16)
        self.dc1 = nn.ConvTranspose2d(conv_dim * 8, conv_dim * 8, kernel_size=4, stride=2, padding=1, bias=False)
        ...
        # (64, 128, 128) -> (24, 128, 128)
        self.c5 = nn.Conv2d(conv_dim * 1, 24, kernel_size=7, stride=1, padding=3, bias=True)
        self.softmax_activation = nn.Softmax(dim=1)

        self.reshape_layer_class = nn.Linear(10, 8 * 8)
    def forward(self, hidden, class_cond_input):
        cls = self.reshape_layer_class(class_cond_input)
        cls = cls.view(cls.size(0), 1, 8, 8)
        h = torch.cat([hidden, cls], dim=1)
        h = self.c0(h)
        ...
        h = self.activation(h)
        h = self.c5(h)
        h = self.softmax_activation(h)
        h = torch.argmax(h, dim=1) 
        return h


class Generator(nn.Module):
    def __init__(self, num_embeddings, embedding_dim=64, z_dim=8, classes_size=10, obj_size=32, clstm_layers=3):
    ...
    def forward(self, imgs, objs, boxes, masks, classes, obj_to_img, z_rand):
        ...
        #print("Recon hidden shape = " + str(h_rec.shape))
        #print("Gen hidden shape = " + str(h_rand.shape))

        img_rec = self.decoder(h_rec, classes)
        img_rand = self.decoder(h_rand, classes)

        print("Image Recon shape = " + str(img_rec.shape))
        print("Image Gen shape = " + str(img_rand.shape))
