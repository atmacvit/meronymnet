...
import json
import numpy as np
from arch.layout2im.layers import build_cnn
from arch.layout2im.layers import GlobalAvgPool
...
class ImageDiscriminator(nn.Module):
    def __init__(self, conv_dim=64, class_cond_dim=10):
        super(ImageDiscriminator, self).__init__()
        self.ch = conv_dim
        self.relu = nn.ReLU(inplace=True)
        self.main = nn.Sequential(
            # (4, 64, 64) -> (64, 32, 32)
            OptimizedBlock(4, self.ch, downsample=True),
            # (64, 32, 32) -> (128, 16, 16)
        ...
        self.cls_pred = nn.Linear(self.ch * 16, class_cond_dim, bias=False)
        self.reshape_linear = nn.Linear(class_cond_dim, 128 * 128)
        # self.apply(weights_init)

    def forward(self, x, cls):
        cls_in = self.reshape_linear(cls)
        cls_in = cls_in.view(cls_in.size(0), 1, 128, 128)
        #print("X image shape " + str(x.shape))
        #print("cls image shape " + str(cls_in.shape))
        x_in = torch.cat([x, cls_in], dim=1)
        h = self.main(x_in)
        h = self.relu(h)
        # (1024, 2, 2) -> (1024,)
        ...
        return output.view(-1), cls


class ObjectDiscriminator(nn.Module):
    def __init__(self, conv_dim=64, n_class=0, class_dim_size=10, downsample_first=False):
        super(ObjectDiscriminator, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        self.main = nn.Sequential(
            # (4, 32, 32) -> (64, 32, 32)
            OptimizedBlock(4, conv_dim, downsample=downsample_first),
            # (64, 32, 32) -> (128, 16, 16)
        ...
        self.classifier_cond = nn.Linear(conv_dim * 16, class_dim_size)
        self.reshape_layer_class = nn.Linear(10, 32 * 32)
    ...
    def forward(self, x, cls, y=None):
        print("cls obj shape " + str(cls.shape))
        in_cls = self.reshape_layer_class(cls)
        in_cls = in_cls.view(in_cls.size(0), 1, 32, 32)
        #print("X obj shape " + str(x.shape))
        #print("cls obj shape " + str(in_cls.shape))
        h = torch.cat([x, in_cls], dim=1)
        h = self.main(h)
        h = self.relu(h)
        # (1024, 2, 2) -> (1024,)
        h = torch.sum(h, dim=(2, 3))
        ...
        output_cond = self.classifier_cond(h)
        return output_src.view(-1), output_cls, output_cond
