...
class ResnetDiscriminator128(nn.Module):
    def __init__(self, num_classes=0, input_dim=3, ch=64):
        ...
        self.block1 = OptimizedBlock(4, ch, downsample=True)
        ...
        self.l8 = nn.utils.spectral_norm(nn.Linear(ch * 16, 10)) 
        ...
        self.reshape_class_layer = nn.utils.spectral_norm(nn.Linear(10, 128 * 128))

    def forward(self, x, cls, y=None, bbox=None):
        b = bbox.size(0)
        # 128x128
        cls_conv = self.reshape_class_layer(cls)
        cls_conv = cls_conv.view(-1, 1, 128, 128)
        
        x = torch.cat((x, cls_conv), dim=1) 
        x = self.block1(x)
        # 64x64
        x1 = self.block2(x)
        # 32x32
...
class CombineDiscriminator128(nn.Module):
    ...
    def forward(self, images, cls, bbox, label, mask=None):
        idx = torch.arange(start=0, end=images.size(0),
                           device=images.device).view(images.size(0),
                                                      1, 1).expand(-1, bbox.size(1), -1).float()
        bbox[:, :, 2] = bbox[:, :, 2] + bbox[:, :, 0]
        ...
        bbox = bbox[idx]
        label = label[idx]
        d_out_img, d_out_obj, d_out_cls = self.obD(images, cls, label, bbox)
        return d_out_img, d_out_obj, d_out_cls
