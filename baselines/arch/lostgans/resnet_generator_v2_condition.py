...
class ResnetGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        num_w = 128+180+10
        self.fc = nn.utils.spectral_norm(nn.Linear(z_dim+10, 4*4*16*ch))
    ...
    def forward(self, z, bbox, cls, z_im=None, y=None):
        ...
        latent_vector = torch.cat((z, label_embedding, cls.repeat(o, 1)), dim=1).view(b, o, -1)
        ...
        # 4x4
        x = self.fc(torch.cat((z_im, cls), dim=1)).view(b, -1, 4, 4)
        # 8x8
        ...
        hh, ww = x.size(2), x.size(3)
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, 1, 1)) # size (b, num_o, h, w)
        var = F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        seman_bbox = torch.sigmoid(seman_bbox) * var
        ...
        # to RGB
        x = self.final(x)
        return x, stage_bbox, seman_bbox, var, alpha4
...
class ResnetGenerator256(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.res6 = ResBlock(ch*2, ch*1, upsample=True, num_w=num_w, predict_mask=True)
        self.final = nn.Sequential(BatchNorm(ch),
                                   nn.ReLU(),
                                   conv2d(ch, output_dim, 3, 1, 1),
                                   nn.Tanh())
    ...
    def forward(self, z, bbox, z_im=None, y=None, include_mask_loss=False):
        ...
        x, mask = self.res6(x, w, stage_bbox)
        # to RGB
        x = self.final(x)
        return x, mask
